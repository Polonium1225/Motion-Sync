...
const databases = {
  ...
  listDocuments: (...args) => safeDatabasesCall('listDocuments', ...args),
  createDocument: (...args) => safeDatabasesCall('createDocument', ...args),
  updateDocument: (...args) => safeDatabasesCall('updateDocument', ...args)
};
...
const safeDatabasesCall = async (method, ...args) => {
  try {
    return await databases[method](...args);
  } catch (error) {
    if (error.code === 1008) {
      console.log("Development origin error - continuing anyway");
      return { documents: [] }; // Return empty mock response
    }
    throw error;
  }
};
