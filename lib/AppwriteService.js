  ...
  try {
    ...
    return await Promise.all(posts.documents.map(async post => {
      ...
      return {
        ...
        user: {
          name: user.name,
          avatar: user.avatar // This should already use your avatar URL method
        },
        // No need to modify imageUrl here as we'll use getPostImageUrl in the component
      };
    }));
  } catch (error) {
  ...
  try {
    const response = await databases.createDocument(
      ...
      ID.unique(),
      {
        userId,
        content,
        imageId: imageFileId, // Store the file ID instead of URI
        likeCount: 0,
        commentsCount: 0
      }
    );
...
export const uploadPostImage = async (imageUri) => {
  try {
    ...
    console.log('Image converted to blob'); // Debug log
    
    // 3. Create a File object
    const file = new File([blob], `post_${Date.now()}.jpg`, {
      type: blob.type || 'image/jpeg'
    });
    console.log('File object created:', file); // Debug log
    
    // 4. Upload to Appwrite
    console.log('Uploading to Appwrite storage...'); // Debug log
    ...
    console.log('Upload successful, file ID:', result.$id); // Debug log
    return result.$id;
  } catch (error) {
