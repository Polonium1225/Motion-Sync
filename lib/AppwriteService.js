 ...
 const getVideo = async (videoId) => {
        ...
        commentsResponse.documents.map(async (comment) => {
          try {
            ...
            if (commentUser?.avatar) {
              avatarUrl = `${API_ENDPOINT}/storage/buckets/profile_images/files/${commentUser.avatar}/view?project=${PROJECT_ID}`;
            }
  ...
  export const addComment = async (postId, userId, content) => {
    try {
      ...
      if (user?.avatar) {
        avatarUrl = `${API_ENDPOINT}/storage/buckets/profile_images/files/${user.avatar}/view?project=${PROJECT_ID}`;
      }
